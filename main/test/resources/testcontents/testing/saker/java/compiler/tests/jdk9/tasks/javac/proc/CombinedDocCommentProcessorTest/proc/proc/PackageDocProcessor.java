...
public class PackageDocProcessor implements Processor {
	...
	private void documentElement(String name) {
		Element elem = elements.getPackageElement(name);
		String doc = elements.getDocComment(elem);
		if (doc == null) {
			return;
		}
		try {
			FileObject outfile = filer.createResource(StandardLocation.SOURCE_OUTPUT, "", "out_pkg_" + name + ".txt",
					elem);
